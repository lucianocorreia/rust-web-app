...
use crate::crypt::{encrypt_into_b64, EncryptContent, Error, Result};
use crate::utils::{
...
pub struct Token {
	...
	pub exp: String,       // Expiiration date in RFC3339
	...
	fn from_str(token_str: &str) -> std::result::Result<Self, Self::Err> {
		...
		Ok(Self {
			ident: b64u_decode(ident_b64u)
				.map_err(|_| Error::TokenConnotDecodeIdent)?,
...
pub fn validate_web_token(origin_token: &Token, salt: &str) -> Result<()> {
	let config = config();
	_validate_token_sign_an_exp(origin_token, salt, &config.TOKEN_KEY)?;
...
fn _validate_token_sign_an_exp(
	origin_token: &Token,
	salt: &str,
	key: &[u8],
) -> Result<()> {
...
fn _token_sign_into_b64u(
	...
	let signature = encrypt_into_b64(
		key,
		&EncryptContent {
			content,
			salt: salt.to_string(),
...
mod tests {
	...
	fn test_token_display_ok() -> Result<()> {
		// Arrange
		let fx_token_str =
			"ZngtaWRlbnQtMDE.MjAyMy0wNS0xN1QxMzoyMDowMFo.some-sign-b64u-encoded";
		let fx_token = Token {
			ident: "fx-ident-01".to_string(),
			exp: "2023-05-17T13:20:00Z".to_string(),
